...
public sealed class EnvironmentQualificationCheck(IConfiguration configuration, HttpClient http) : IQualificationCheck
{
    public async Task<QualificationReport> Run(JsonElement manifest, string directory, string reportPath, CancellationToken ct)
    {
        ...
        if (new SqlConnectionStringBuilder(connection).InitialCatalog != expectedDatabase) throw new ArgumentException("The configured SQL database differs from expectedDatabase.");
        ...
        observations.Add(new("SQL schema", true, "Existing database is accessible with no pending migrations. No migration or write was performed.", new() { ["database"] = expectedDatabase, ["migrations"] = (await database.Database.GetAppliedMigrationsAsync(ct)).ToArray() }));
