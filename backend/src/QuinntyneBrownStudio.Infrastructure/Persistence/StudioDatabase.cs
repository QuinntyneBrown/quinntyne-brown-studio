...
public sealed class StudioDatabase(StudioDbContext db) : IStudioDatabase
{
    public async Task Verify(CancellationToken cancellationToken = default)
    {
        ...
        if (!await db.Database.CanConnectAsync(cancellationToken))
            throw new InvalidOperationException("The configured LocalDB database is inaccessible. Check LocalDB installation, instance, and Windows account; run QuinntyneBrownStudio.Api --migrate explicitly if the application database does not exist.");
        try
        {
            if ((await db.Database.GetPendingMigrationsAsync(cancellationToken)).Any())
                throw new InvalidOperationException("LocalDB has unapplied migrations. Back up the database and run QuinntyneBrownStudio.Api --migrate before starting the API or worker.");
        }
        catch (SqlException)
        {
            throw new InvalidOperationException("Cannot verify the LocalDB schema. Check database access and run QuinntyneBrownStudio.Api --migrate explicitly before starting.");
        }
    }
