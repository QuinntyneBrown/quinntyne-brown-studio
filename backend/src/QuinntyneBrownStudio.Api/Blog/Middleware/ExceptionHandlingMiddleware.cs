...
public class ExceptionHandlingMiddleware(RequestDelegate next, ILogger<ExceptionHandlingMiddleware> logger, IHostEnvironment env)
{
    ...
    private static async Task HandleExceptionAsync(HttpContext context, Exception ex, IHostEnvironment env)
    {
        var (status, title, detail, errors) = ex switch
        {
            ...
            UnprocessableEntityException ue422 => (422, "Unprocessable Entity", ue422.Message, (Dictionary<string, string[]>?)null),
            ...
            Microsoft.EntityFrameworkCore.DbUpdateConcurrencyException => (412, "Precondition Failed", "The article has changed. Refresh before saving.", (Dictionary<string, string[]>?)null),
            Microsoft.EntityFrameworkCore.DbUpdateException => (409, "Conflict", "The change conflicts with existing data. Refresh before trying again.", (Dictionary<string, string[]>?)null),
            Microsoft.Data.SqlClient.SqlException => (503, "Service Unavailable", "The blog database is temporarily unavailable.", (Dictionary<string, string[]>?)null),
