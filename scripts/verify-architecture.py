...
for name, allowed in [('Domain', []), ('Application', ['Domain'])]:
    text = (ROOT / f'backend/Qbs.{name}/Qbs.{name}.csproj').read_text(encoding='utf-8-sig')
...
check('12.5.0' in (ROOT / 'backend/Qbs.Application/Qbs.Application.csproj').read_text(), 'MediatR license baseline changed')
for source in (ROOT / 'backend/Qbs.Api').rglob('*.cs'):
...
for project in ['marketing', 'admin', 'client', 'design-system', 'components', 'domain', 'api', 'application']:
