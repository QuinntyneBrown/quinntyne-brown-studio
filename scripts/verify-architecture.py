...
check('12.5.0' in (ROOT / 'backend/src/QuinntyneBrownStudio.Application/QuinntyneBrownStudio.Application.csproj').read_text(encoding='utf-8-sig'), 'MediatR license baseline changed')
for source in (ROOT / 'backend/src/QuinntyneBrownStudio.Api').rglob('*.cs'):
    if 'obj' in source.parts: continue
    check(not re.search(r'\.Map(?:Get|Post|Put|Delete|Patch)\(', source.read_text(encoding='utf-8-sig')), f'Minimal API product handler: {source}')
manifest = json.loads((ROOT / 'frontend/component-catalog.json').read_text(encoding='utf-8'))
