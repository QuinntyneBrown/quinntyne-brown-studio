...
AC-L2-070-08 through AC-L2-070-12 cover the studio listing introduced on 2026-09-11. Both public listing routes render the shared `_BlogListing.cshtml` partial and the marketing variant of `_Layout.cshtml`.
...
- AC-L2-070-11: API acceptance checks that the listing ships the backdrop element and the close icon in place of the former text label. Browser acceptance, through `BlogListPage.assertOverlayMenu`, checks at 390 and 768 pixels that the closed control shows the menu icon and reads `Open menu`, that opening it reveals a backdrop whose computed background is neither fully transparent nor fully opaque, that the control swaps to the close icon and reads `Close menu`, and that clicking the backdrop closes the overlay and restores the menu icon. Review of the [layout](../../backend/src/QuinntyneBrownStudio.Api/Pages/Shared/_Layout.cshtml) confirms Escape closes the overlay through the same native button.
- AC-L2-070-12: browser acceptance, through `BlogListPage.assertOverlayMenu`, checks at 390 and 768 pixels that the open overlay begins at the top edge of the viewport: the header sits at the top of the page, the backdrop starts no higher than the bottom of the header, and neither side of the band beside the header is covered by the backdrop. The overlay therefore presents the brand, the close control, and the menu links on one undimmed surface, with the listing dimmed below it.
