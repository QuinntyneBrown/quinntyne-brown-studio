...
These are proposed production contracts. The feature pages define the request fields and outcomes for their routes. [Architecture](architecture.md) supplies wire conventions, authorization, and transactions; [decisions](../specs/decisions.md) supplies policy values.
...
| `/api/admin/discounts` | `DiscountsController` | Rule administration; public eligibility remains part of quote calculation. |
| `/api/admin/rates` | `RatesController` | Rate configuration and revision. |
| `/api/admin/studios`, `/api/public/studios` | `StudiosController` | Private configuration and enabled public projection. |
...
| Public gallery, content, promotion, equipment, vendor, analysis, retention, print-option, print-request, album, and authentication routes | Corresponding controller named in the feature page | Each controller retains its declared feature route family. |

`IQuotesApi`, `IDiscountsApi`, `IUploadsApi`, `IPhotosApi`, and `IClientGalleriesApi` are separate consumer contracts. `IClientGalleriesApi` delegates private image retrieval to `IPhotosApi`. The `PhotosController` route still has a single owner. Every consumer contract has a separately declared injection token and replaceable HTTP/mock implementations.
...
`IStudioStore` exposes scoped queries and aggregate operations rather than an unrestricted database connection. Queries include gallery membership and retention predicates when handling client data. `BeginTransaction()` returns `IUnitOfWork`, whose `Commit()` persists aggregate changes and outbox records together. Transaction disposal rolls back uncommitted work. Query-only domain results such as `QuoteCalculation` and `AvailabilityResult` are not stored aggregates.
