...
Uploads use private block blobs, 8 MiB blocks, and at most four active file transfers per browser. The application issues short-lived grants for one generated blob name and a batch-owned photo identifier. Grants cannot list or read other blobs. The browser retains batch identifiers and committed-block progress; resuming after a reload requires reselection of matching local files. Finalization checks storage length, content, and association before queueing conversion. Success in transferring bytes is distinct from preview readiness.
